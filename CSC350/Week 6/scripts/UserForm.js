...
function saveUserForm() {
  if (checkUserForm()) {
    var user = {
      ...
      "BoilerID": $("#boilerdNumber").val(),
      "NewPassword": $("#changePassword").val(),
      "DOB": $("#datePurchase").val(),
...
function showUserForm() { //Load the stored values in the form
  ...
  if (user != null) {
    ...
    $("#boilerdNumber").val(user.BoilerID);
    $("#changePassword").val(user.NewPassword);
    $("#datePurchase").val(user.DOB);
	  $("#maxPress").val(user.MaxPress);
    $("#minPress").val(user.MinPress);
